...
function shortcode_adverts_add( $atts ) {
    ...
    if( $post_id>0 && get_post( $post_id )->post_author == get_current_user_id() ) {
        ...
        $post = get_metadata( 'post_type', 'adverts', 'adverts_address1' );

        $bind["adverts_person"] = wp_get_current_user()->display_name;
        $bind["adverts_email"] = wp_get_current_user()->user_email;
        $bind["adverts_phone"] = wp_get_current_user()->adverts_phone;

        $user_id = wp_get_current_user()->id;
        $user_info = get_userdata($user_id);

        $bind["adverts_address1"] = $user_info->address1;
        $bind["adverts_address2"] = $user_info->address2;
        $bind["adverts_postcode"] = $user_info->postcode;
    }
